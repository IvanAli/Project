...
all_names = json.load(open("../all_names.json"))
...
class GetUserID(Resource):
  ...
  def get(self, args):
    ...
    vertices = find_vertex(graph, graph.vertex_properties["name"], name)
    if vertices:
      return graph.vertex_index[vertices[0]]
    return -1

api.add_resource(ShortestPath, '/shortest_path')
api.add_resource(WholeGraph, '/whole_graph')
api.add_resource(AllNames, '/all_names')
api.add_resource(InducedSubgraph, '/induced_subgraph')
api.add_resource(Test, '/test')
api.add_resource(GetUserID, '/get_user_id')
